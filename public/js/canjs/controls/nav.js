	...
	App.Controls.Nav = can.Control.extend({
	  ...
	  // Helper function for opening and closing the menu using CSS3.
		...
		':folio/:project route' : function(data) {
			...
			this.showNavHalf(data.folio);
			if (data.folio == 'film') this.initScreen(data);
		},

		// Install the video player and initialize VideoJS.
		initScreen: function(data) {
			var project = $('.film .' + data.project + ' .theater');
			var video = project.attr('data-theater-video');
			var poster = project.attr('data-theater-poster');
			console.log(video, poster);
			var dom = can.view('video-player', { 
				section: data.folio, 
				project: data.project, 
				poster: poster,
				video: video
			});
			var screenage = project.find('.screen').append(dom);
			setTimeout(function() {
				videojs(screenage.find('video')[0], { "controls": true, "loop": false, "autoplay": false }, function() {
